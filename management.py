...
#=======================================Exit================================================
def Exit():
    result = tkMessageBox.askquestion('Python Inventory Management System','Are you want to Exit ?', icon="warning")
    if result == 'yes':
        root.destroy()
        exit()
...
def Home():
    ...
    Title.pack(pady=10)
    lbl_display = Label(Title, text="Python Inventory Management System", font=('arial', 35))
    lbl_display.pack()
    menubar = Menu(Home)
...
def AddNewForm():
    ...
    lbl_productname = Label(MidAddNew, text="Product Name:", font=('arial', 26), bd=10)
    lbl_productname.grid(row=0, sticky=W)
    lbl_qty = Label(MidAddNew, text="Product Quantity:", font=('arial', 26), bd=10)
    lbl_qty.grid(row=1, sticky=W)
    lbl_price = Label(MidAddNew, text="Product Price:", font=('arial', 26), bd=10)
    lbl_price.grid(row=2, sticky=W)
    productname = Entry(MidAddNew, textvariable=PRODUCT_NAME, font=('arial', 26), width=15)
    productname.grid(row=0, column=1)
    productqty = Entry(MidAddNew, textvariable=PRODUCT_QTY, font=('arial', 26), width=15)
    productqty.grid(row=1, column=1)
    productprice = Entry(MidAddNew, textvariable=PRODUCT_PRICE, font=('arial', 26), width=15)
    productprice.grid(row=2, column=1)
    btn_add = Button(MidAddNew, text="Save", font=('arial', 22), width=30, bg="#009ACD", command=AddNew)
...
def Delete():
    if not tree.selection():
       print("ERROR")
        ...
        result = tkMessageBox.askquestion('Python Inventory Management System','Are you want to delete with Record?', icon="warning")
        if result == 'yes':
            curItem = tree.focus()
...
def ShowView():
    global viewform
    viewform = Toplevel()
    viewform.title("Python Inventory Management System/View Prodcut")
    width = 600
    height = 400
    screen_width = Home.winfo_screenwidth()
    screen_height = Home.winfo_screenheight()
...
#====================================Log Out and Login=================================================================
def Logout():
    result = tkMessageBox.askquestion('Python Inventroy Management System',' Are you want to  logout', icon="warning")
    if result == 'yes': 
        admin_id = ""
        root.deiconify()
        Home.destroy()
